...
Before the first detect, ask which fields to detect: **board fields**,
**signer-fillable placeholders**, or **both**. Pass that as `intent`. Do not
pick silently. If the boxes are wrong they can describe the fix in text
(`refine=true` + `intent`) or start over (`force=true` — the next save
replaces the old fields).

AI detect finds signer-fillable pads **and** visual board widgets when you pass
`envelope_id` + `item_id`. Pass those placeholders through on save. Printed
`{{Column Title}}` / `{columnId}` tags are not rewritten.
